...
try:
    ...
    # =====================================================
    # FGUSERS TABLE (UPDATED STRUCTURE)
    # =====================================================
    ...
    CREATE TABLE IF NOT EXISTS fgusers (
        ...
        access SET('CAROUSEL','VIDEOS','UPLOAD'),
    ...
    for _, row in fgusers_df.iterrows():
        ...
        cursor.execute(insert_fg_query, (
            row['user'],
            row['password'],   # still plain text for now
            row['access'],
            bool(row['faces'])  # converts 0/1 or TRUE/FALSE properly
        ))
